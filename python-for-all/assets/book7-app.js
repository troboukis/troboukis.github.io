...
# Figures are captured by the JS layer after each cell run; suppress the
# "non-GUI backend" warning that plt.show() would otherwise emit.
...
def _capture_figures():
    ...
    for num in plt.get_fignums():
        ...
        buf.seek(0)
        figs.append(_b64.b64encode(buf.read()).decode("utf-8"))
    plt.close("all")
    return figs
`);
    ...
    const PY_BUILTINS = new Set([
      ...
      "isinstance", "plt", "np", "ax", "fig", "axes", "json", "pandas", "pd"
    ]);
    ...
    async function runCell(cellEl) {
      ...
      // Remove any previously rendered chart images
      out.querySelectorAll("img.chart-out").forEach(img => img.remove());
      currentOut = out;
...
`;
        ...
        try {
          const pyFigs = pyodide.runPython("_capture_figures()").toJs();
          if (pyFigs && pyFigs.length > 0) {
            for (const imgData of pyFigs) {
              const img = document.createElement("img");
              img.src = `data:image/png;base64,${imgData}`;
              img.className = "chart-out";
              img.style.cssText = "max-width:100%;display:block;margin-top:8px;border-radius:4px;";
              out.appendChild(img);
            }
          }
        } catch (_) { /* no figures */ }
    ...
    document.querySelectorAll(".cell:not(.cell-static)").forEach((cell) => {
    ...
    document.querySelectorAll(".cell-static").forEach(cell => {
      ...
      if (btn && ta) {
        btn.addEventListener("click", async () => {
          ...
          const status = cell.querySelector(".copy-status");
          if (status) {
